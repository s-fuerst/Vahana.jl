...
"""
function add_agenttype!(types::ModelTypes, ::Type{T}, ::Type{C} = Dict;
                 size::Int64 = 0) where { T, C }
...
add_agenttype!(t::Type{T}, c::Type{C}; kwargs...) where T where C =
...
"""
function add_edgetype!(types::ModelTypes, ::Type{T}, ::Type{C} = Dict;
                size::Int64 = 0) where T where C 
    @assert !(Symbol(T) in keys(types.edges)) "Each type can be added only once"
    @assert isbitstype(T)
    types.edges[Symbol(T)] = Symbol(C)
...
add_edgetype!(t::Type{T}, c::Type{C}; kwargs...) where T where C =
