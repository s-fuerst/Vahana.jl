...
@testset "Initialization" begin
    ...
    @test length(sim.agents[sim.next]) == 1
    @test sim.type2number[Person] == 1
    @test sim.number2type[1] == Person

    add_agenttype!(sim, HH)
    @test length(sim.agents[sim.next]) == 2
    @test sim.type2number[HH] == 2
    @test sim.number2type[2] == HH

    p1id = add_agents!(sim, p1)
    @test length(sim.agents[sim.next][Person]) == 1
    @test sim.agents[sim.next][Person][p1id] == p1

    ids = add_agents!(sim, p1, p2)
    p2id = ids[2]
    @test length(sim.agents[sim.next][Person]) == 3
    @test sim.agents[sim.next][Person][p2id] == p2
